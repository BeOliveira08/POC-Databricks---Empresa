...
GOLD_FATO_NOTAS_FISCAIS = f"{CATALOG}.{GOLD}.fato_notas_fiscais"
...
df_gold = (
    spark.table(SILVER_TB_NOTAS_FISCAIS).alias("n")
    .join(df_data_competencia, F.col("competencia").cast("date") == F.col("competencia_ref"), "left")
    .join(df_data_emissao, F.col("data_emissao").cast("date") == F.col("data_emissao_ref"), "left")
    ...
    .select(
        ...
        F.col("n.valor_servicos").cast("double").alias("valor_servicos"),
        F.col("n.valor_deducoes").cast("double").alias("valor_deducoes"),
        F.col("n.valor_iss").cast("double").alias("valor_iss"),
        F.col("n.valor_liquido").cast("double").alias("valor_liquido"),
...
w_nota = Window.partitionBy("id_nota_fiscal").orderBy(
...
df_gold = (
    df_gold
    .withColumn("rn_nota", F.row_number().over(w_nota))
    .filter(F.col("rn_nota") == 1)
    .drop("rn_nota")
)

spark.sql(f"""
CREATE TABLE IF NOT EXISTS {GOLD_FATO_NOTAS_FISCAIS} (
...
write_overwrite(df_gold, GOLD_FATO_NOTAS_FISCAIS)
...
apply_governance(GOLD_FATO_NOTAS_FISCAIS)
