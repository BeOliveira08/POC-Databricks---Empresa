...
def _current_notebook_dir():
    try:
        ...
        return "/Workspace/Shared/portfolio-lakehouse-template/pipelines/notebooks/silver"
...
print("=" * 100)
print(f"Silver pipeline iniciado em {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
if any(r["status"] == "ERRO" for r in results):
    raise Exception(f"Silver pipeline finalizado com erro(s). Tempo total: {total_duration}s")

print("\n[OK] Silver pipeline finalizado com sucesso.")
